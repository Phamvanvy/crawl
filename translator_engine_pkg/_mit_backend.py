...
6b. TYPE TAG (REQUIRED, right after <|n|>, exactly one): [speech]=lời nói trong bong bóng (MẶC ĐỊNH — phân vân thì chọn cái này) | [thought]=suy nghĩ/độc thoại nội tâm/cảm giác cơ thể của nhân vật (vd "Toàn thân nóng rực", "Có tác dụng chưa?") — kể cả khi nằm trong bong bóng | [moan]=rên/khoái cảm (haa,hừ,ah♡) | [shout]=hét to | [anger]=giận dữ/gắt gỏng/đe doạ (cáu nhưng không nhất thiết gào; nếu vừa giận vừa hét → ưu tiên [anger]) | [fear]=sợ hãi/run rẩy/lắp bắp ("K-không thể…","Đ-đừng lại gần!") | [narration]=CHỈ lời NGƯỜI KỂ thuật lại bối cảnh/diễn biến (giọng kể khách quan, thường ngôi thứ 3) — TUYỆT ĐỐI không dùng cho suy nghĩ, cảm giác hay lời nói của nhân vật | [sfx]=tượng thanh THUẦN (câu có CẢ tiếng cười/rên LẪN lời nói → phân loại theo LỜI NÓI [speech]/[thought], KHÔNG phải [sfx]/[moan]) | [title]=tiêu đề bộ truyện/chương & chữ TRANG TRÍ (thư pháp, con dấu, logo truyện — thường kèm số chương/phần 一/二/3/（五）). HAI trường hợp: (a) tiêu đề ĐỌC ĐƯỢC thành tên/cụm CÓ NGHĨA (tên bộ, tên chương, phụ đề — vd 宠物捕捉大师3 → "Bậc Thầy Bắt Thú Cưng 3") → DỊCH sang tiếng Việt NGAY SAU nhãn, GIỮ số chương/phần: "<|n|>[title] Bậc Thầy Bắt Thú Cưng 3" (vùng SẼ được render lên ảnh). (b) CHỈ khi là chữ trang trí thuần/con dấu/logo/OCR rác Hán vô nghĩa KHÔNG đọc thành chữ → trả TRƠ "<|n|>[title]" KHÔNG kèm chữ nào (vùng bị xoá khỏi ảnh, không render). CẢNH BÁO: chữ thư pháp/cách điệu mà vẫn là CỤM/CÂU CÓ NGHĨA kể chuyện (lời dẫn cảnh, khẩu hiệu, phụ đề dưới khung — vd 于逆星之下举起义旗 → "Dưới Nghịch Tinh, giương cao cờ nghĩa.") thì KHÔNG PHẢI [title] — PHẢI DỊCH với nhãn [narration]; gán nhầm [title] sẽ làm MẤT lời truyện khỏi ảnh. Ví dụ: <|1|>[thought] Đầu óc cứ bồng bềnh… | <|2|>[anger] Mày dám! | <|3|>[fear] X-xin… đừng… | <|4|>[narration] Ba ngày sau, tại học viện… | <|5|>[title] Bậc Thầy Bắt Thú Cưng 3
...
class MITImageTranslator:
    ...
    def _spawn_mit(self, cmd, stop_event: threading.Event | None = None, extra_env: dict | None = None):
        ...
        # OCR đọc vớt: 48px đọc chuẩn nhưng loại hẳn vùng dưới ngưỡng prob (bỏ sót
        # bong bóng khó) → cho 48px_ctc đọc riêng các vùng 48px loại (patch
        # ocr/__init__.py đọc env này). Chỉ áp khi model chính là 48px (mặc định
        # khi để trống); chọn 48px_ctc/mocr thuần thì giữ hành vi gốc.
        if (self.ocr_model or "48px") == "48px":
            sub_env["MIT_OCR_FALLBACK"] = "48px_ctc"
            self._log("  [OCR] 48px + CTC đọc vớt vùng bỏ sót")
