...
6c. CÂU BỊ CẮT ĐÔI → [cont]: Nếu nhiều segment LIÊN TIẾP thực chất là MỘT câu liền mạch bị cắt giữa chừng (segment trước kết thúc lửng, segment sau nối thẳng vào — vd <|3|>…对文思 + <|4|>月进行…), dịch TRỌN VẸN cả câu vào segment ĐẦU TIÊN, các segment sau chỉ ghi đúng "[cont]" (không thêm chữ nào khác). TUYỆT ĐỐI KHÔNG dồn bản dịch của các câu sau lên slot trước — mỗi segment còn lại vẫn giữ đúng bản dịch của chính nó. Ví dụ: <|3|>[narration] Nói thật là… liền bế quan. | <|4|>[cont] | <|5|>[narration] Cục diện hỗn loạn…
